...
int main() {
    scc::SynthEngine synth(44100, 4); // tiny voice count to force stealing
    for (int i = 0; i < 4; i++)
    ...
    synth.render(buf.data(), 256); // let envelopes ramp up before stealing
    ...
    // maxVoices resize should not crash and should re-clamp count
